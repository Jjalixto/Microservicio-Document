...
class OneOwner(ContractStrategy):
    ...
    def process_request(request: DocumentRequest):
        #Carga del documento
        document = DocxTemplate('lib/Contract-Single-One.docx')
        condicion = OneOwner.validacion_condicion(request, document)
    ...
    def validacion_condicion(request: DocumentRequest, document: Document):
        ...
        return OneOwner.tipo_condicion(request.condicion, request, document)
    
    @staticmethod
    def tipo_condicion(condicion: str, request: DocumentRequest, document: Document):
        if condicion == 'contado':
            return OneOwner.counted_type(request, document)
        elif condicion == 'financiado':
            return OneOwner.financed_type(request, document)
        elif condicion == 'fraccionado':
            return OneOwner.fractionated_type(request, document)
        else:
    ...
    def counted_type(request: DocumentRequest, document: Document):
        ...
        OneOwner.actualizar_excel(ruta_excel, request)
        
        # Leer datos del archivo Excel
        tabla_datos = OneOwner.leer_datos_excel(ruta_excel)
        OneOwner.actualizar_documento_word(ruta_word, tabla_datos)
    ...
    def financed_type(request: DocumentRequest, document: Document):
        ...
        OneOwner.insertar_texto_estatico(document)
        OneOwner.reemplazar_marcadores(document, valores)
        ...
        # Leer datos del archivo Excel
        tabla_datos = OneOwner.leer_datos_excel(ruta_excel)
        parametros = OneOwner.getCampoEspecifico(ruta_excel)
        OneOwner.actualizar_documento_word(ruta_word, tabla_datos, parametros)
    ...
    def fractionated_type(request: DocumentRequest, document: Document):
        ...
        #para todo precio dentro del contrato tiene que ir con 2 decimales example: 50,000.00 y sin corchetes
        #plomo claro para la cabezera del cronograma de pagos
        #cronograma [fecha vencimiento] -> DD-MM-YY
        'precio_number': f"{request.monto_venta:,.2f}" if request.monto_venta else '',
        'precio_letras': request.precio_letras or '',
        'cuota_number': f"{request.cuota_inicial:,.2f}" if request.cuota_inicial else '',
        'cuo_init_letras': request.cuota_inicial_letras or '',
        }
        
        document.render(valores)
        
        OneOwner.insertar_texto_estatico_fractionated(document)
        OneOwner.reemplazar_marcadores(document, valores)
    ...
    def actualizar_documento_word(ruta_archivo_word, tabla_datos, parametros):
        ...
        OneOwner.reemplazar_campos_especificos(doc, parametros)
        OneOwner.agregar_tabla_word(doc, tabla_datos)
